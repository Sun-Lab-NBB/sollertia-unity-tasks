...
namespace Gimbl
{
    ...
    public class MQTTChannel
    {
        ...
        public void Init(string topicStr, bool isListener, byte lvl)
        {
            ...
            if (isListener)
            {
                client.Subscribe(this, topic, lvl);
            }
            ;
        }
    ...
    public class MQTTRawChannel : MQTTChannel
    {
        ...
        // Inherit Constructor
        public MQTTRawChannel(string topicStr, bool isListener = true, byte lvl = MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE)
            : base(topicStr, isListener, lvl) { }
        ...
        {
            client.client.Publish(topic, Encoding.UTF8.GetBytes(msg));
        }
    }
}

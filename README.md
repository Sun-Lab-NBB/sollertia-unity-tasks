...
task and modify it to match the desired parameters. Use ctrl/cmd D to duplicate existing segment prefabs and .json
files.
...
- **cues** *(array\<Cue>)*: The list of all cues from any segment. The order of this list determines the integer id's
  assigned to each cue during cue sequence logging.
    - **Cue**
        ...
        - **length** *(number, Unity units)*: The wall space, in unity units, taken up by the cue (the length of the
          cue).
...
- **segments** *(array\<Segment>)*: The list of all segments.
    - **Segment**
        ...
        - **cue_sequence** *(string[])*: The ordered list of cues in the segment. The sum of cue lengths in this list
          must
          match the total length of the segment prefab, in Unity units.
        - **transition_probabilities** *(number[])*: The ordered list of probabilities of choosing each possible
          successor
          segment. All probabilities in the list must sum to 1. The length of this list must correspond to the total
          number
...
4. The *Task* script contains additional parameters which should not need to be modified:
    - **Require Lick**: Determines whether the animal has to lick within the stimulus trigger zone to get the stimulus.
      If disabled,
      the animal gets the stimulus by entering the stimulus zone and colliding with the invisible stimulus boundary
      wall.
    ...
    - **Visible Boundary**: Determines whether to reveal the typically hidden stimulus trigger zone collision boundary
      to the
